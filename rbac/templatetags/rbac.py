...
def show_menu(request):
    ...
    per_side_list = request.session.get('per_side_list')
    # print(per_side_list)
    # per_side_list = [
    ...
    per_menu_dict = {}
    for dict_item in per_side_list:
        if not dict_item.get('menu_ref'):
            per_menu_dict[dict_item.get('url_id')] = dict_item.copy()
            # per_menu_dict[dict_item.get('url_id')] = dict_item
    # print(per_menu_dict)
    # per_menu_dict = {
    ...
    for dict_item in per_side_list:
        ...
        if re.match(re_url, current_url):
            ...
            if not menu_ref_id:
                per_menu_dict[dict_item.get('url_id')]['active'] = True
            else:
                per_menu_dict[menu_ref_id]['active'] = True
    # print(per_menu_dict)
    # per_menu_dict = {
    #     1: {'url_id': 1, 'url_title': '用户信息', 'url': '/userinfo/', 'menu_ref': None, 'menu_id': 1, 'menu_title': '用户组',
    #         'active': True},
    ...
    side_dict = {}
    for dict_item in per_menu_dict.values():
